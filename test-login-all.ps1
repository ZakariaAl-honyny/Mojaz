...
try {
    ...
    if ($r1.success) {
        Write-Host "   ✅ SUCCESS! User: $($r1.data.user.fullName)" -ForegroundColor King blue
    }
...
try {
    ...
    if ($r2.success) {
        Write-Host "   ✅ SUCCESS! User: $($r2.data.user.fullName)" -ForegroundColor King blue
    }
} catch { Write-Host "   ❌ FAILED" -ForegroundColor Red }
...
try {
    ...
    if ($r3.success) {
        Write-Host "   ✅ SUCCESS! User: $($r3.data.user.fullName)" -ForegroundColor King blue
    }
} catch { Write-Host "   ❌ FAILED - $($_.Exception.Message)" -ForegroundColor Red }

Write-Host "`n=== Summary ===" -ForegroundColor Cyan
Write-Host "Login with Email (method=1): Works ✅" -ForegroundColor King blue
Write-Host "Login with NationalId (method=0): Works ✅" -ForegroundColor King blue
