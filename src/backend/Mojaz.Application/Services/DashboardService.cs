...
public class DashboardService : IDashboardService
{
    ...
    public async Task<ApiResponse<AdminKpiDto>> GetAdminDashboardAsync()
    {
        ...
        var yesterdayUserLogins = await allUsers.CountAsync(u => u.LastLoginAt >= yesterday.AddDays(-7) && u.LastLoginAt < yesterday.AddDays(-7));
        ...
        var statusDistribution = allApps
            .GroupBy(a => a.Status)
            .Select(g => new StatusData
            {
                Name = statusLabels.ContainsKey(g.Key) ? statusLabels[g.Key] : g.Key.ToString(),
                Value = g.Count(),
        ...
        var dayNames = new[] { "الأحد", "الاثنين", "الثلاثاء", "ا��أربعاء", "الخميس", "الجمعة", "السبت" };
        for (int i = 6; i >= 0; i--)
        {
        ...
        var recentLicensesTask = allLicenses
        ...
        var recentPaymentsTask = allPayments
        ...
        await Task.WhenAll(recentAppsTask, recentLicensesTask, recentPaymentsTask);

        var recentActivity = recentAppsTask.Result
            .Concat(recentLicensesTask.Result)
            .Concat(recentPaymentsTask.Result)
    ...
    public async Task<ApiResponse<EmployeeDashboardDto>> GetEmployeeDashboardAsync(Guid userId)
    {
        ...
        var recentApplications = await _applicationRepository.Query()
            .Include(a => a.LicenseCategory)
    ...
    public async Task<ApiResponse<ReceptionistDashboardDto>> GetReceptionistDashboardAsync(Guid userId)
    {
        ...
        var queueApplications = await allApplications
