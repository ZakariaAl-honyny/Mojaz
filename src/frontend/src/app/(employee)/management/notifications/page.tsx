...
export default function EmployeeNotificationsPage() {
  ...
  const canAccess = user?.role && (['Admin', 'Manager', 'Receptionist'] as string[]).includes(String(user.role));
              ...
              <CardContent>
                ...
                <div className="mt-4 p-4 bg-primary-50 rounded-xl">
                  ...
                  {sendToType === 'single' && selectedUser && (
                    <p className="text-sm text-neutral-600">
                      {users.find(u => u.id === selectedUser)?.fullName} ({users.find(u => u.id === selectedUser)?.email})
                    </p>
                  )}
