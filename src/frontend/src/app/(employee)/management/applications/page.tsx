...
export default function EmployeeApplicationsPage() {
  ...
  const { data: applicationsData, isLoading } = useQuery({
    ...
    queryFn: () => ApplicationService.getApplications({ page: 1, pageSize: 50 }),
  });
...
return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 space-y-12 py-6">
...
<Button variant="outline" className="h-10 md:h-12 px-5 md:px-6 rounded-md border-neutral-100 bg-white font-bold gap-2 md:gap-3 transition-all">
             <Filter className="h-4 w-4 md:h-5 md:w-5 text-neutral-400" />
             Filter
           </Button>
        </div>
      </div>
              ...
              <tbody>
                {table.getRowModel().rows.length > 0 ? (
                  table.getRowModel().rows.map((row) => (
                    <tr
                      key={row.id}
                      className="group border-b border-neutral-50 last:border-0 hover:bg-neutral-50/50 transition-all"
                    >
...
<div className="p-10 flex items-center justify-between border-t border-neutral-50 bg-neutral-50/20">
            <div className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.2em] italic">
              Showing <span className="text-neutral-900">{table.getRowModel().rows.length}</span> of <span className="text-neutral-900">{applicationsData?.data?.totalCount ?? 0}</span> processing units
            </div>
            <div className="flex items-center gap-3">
              <Button
                variant="outline"
