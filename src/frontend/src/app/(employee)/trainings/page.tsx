...
export default function EmployeeTrainingPage() {
  ...
  // Fetch training records from API - for now we fetch all applications and show their training status
  const { data: trainingData, isLoading } = useQuery({
    queryKey: ['training-records'],
    queryFn: async () => {
      // For MVP, fetch applications and get training info for each
      // This is a simplified approach - in production, there would be a dedicated endpoint
      const response = await fetch('/api/v1/training/all');
      return response.json();
    },
    enabled: false, // Disabled until endpoint is implemented
  });

  // For display - use empty array initially, will be populated when API is ready
  const [trainees] = useState<any[]>([]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 space-y-8 py-6">
