...
import type { NotificationType, NotificationDto } from '@/types/notification.types';
...
const EVENT_TO_TYPE: Record<string, NotificationType> = {
  ...
  MedicalExamScheduled: 'appointment',
  MedicalExamCompleted: 'appointment',
  TheoryTestScheduled: 'appointment',
  TheoryTestCompleted: 'appointment',
  PracticalTestScheduled: 'appointment',
  PracticalTestCompleted: 'appointment',
  ApplicationStatusChanged: 'status',
  LicenseIssued: 'status',
};

const getType = (eventType: string): NotificationType =>
  EVENT_TO_TYPE[eventType] ?? 'system';

const getNotificationIcon = (eventType: string, className?: string) => {
  const type = getType(eventType);
  const cls = cn('w-5 h-5', className);
  switch (type) {
    case 'payment':     return <CreditCard className={cls} />;
    case 'appointment': return <Calendar className={cls} />;
    case 'status':      return <FileText className={cls} />;
...
const getIconBg = (eventType: string) => {
  switch (getType(eventType)) {
    ...
    case 'appointment': return 'bg-blue-50 text-[#1a3a8f] border-blue-100';
...
export default function NotificationsPage() {
  ...
  const {
    ...
    markAsRead,
    markAsReadLoading,
  } = useNotifications({ page: 1, pageSize: 50, enabled: true });

  // ─── Filter logic ────────────────────────────────────────────────────────
  const filteredNotifications = notifications.filter((n: NotificationDto) => {
    if (filter === 'unread' && n.isRead) return false;

    if (filter !== 'all' && filter !== 'unread') {
      if (getType(n.eventType) !== filter) return false;
    }

    if (searchQuery) {
      const hay = `${n.titleAr} ${n.messageAr} ${n.titleEn} ${n.messageEn}`.toLowerCase();
    ...
    <div className="space-y-10 font-arabic pb-12" dir="rtl">
      ...
      <div className="flex flex-col xl:flex-row gap-6 justify-between bg-white border border-neutral-200 p-8 rounded-[2.5rem] shadow-sm mx-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {FILTER_TABS.map((tab) => (
            ...
            >
              {'icon' in tab && <tab.icon className="w-3.5 h-3.5 opacity-60" />}
              {tab.label}
              {'count' in tab && (
                <span className={cn(
                  'min-w-[1.25rem] h-[1.125rem] px-1 rounded-md flex items-center justify-center text-[8px] font-black',
      ...
      <div className="grid grid-cols-1 gap-4 px-4 pb-12">
        {notificationsLoading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="font-black text-neutral-300 text-xs uppercase tracking-widest animate-pulse">جاري التحميل...</p>
          </div>
        ) : (
          <AnimatePresence mode="popLayout">
            {filteredNotifications.length > 0 ? (
              filteredNotifications.map((n: NotificationDto, idx: number) => (
                <motion.div
                  key={n.id}
                  layout
                    ...
                    <CardContent className="p-7">
                      <div className="flex items-start gap-6">
                        <div className={cn(
                          ...
                          getIconBg(n.eventType)
                        )}>
                          {getNotificationIcon(n.eventType, 'w-7 h-7')}
                        </div>
                        ...
                        <div className="flex-1 space-y-2 text-right">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                              ...
                              {!n.isRead && (
                                <Badge className="bg-[#1a3a8f] text-white px-2 py-0.5 rounded-lg font-black text-[9px] uppercase tracking-widest leading-none">
                                  جديد
                                </Badge>
                              )}
                            </div>
      ...
      <div className="flex justify-center pt-4 opacity-30 select-none">
        <div className="flex items-center gap-4 py-3 px-6 rounded-full border border-neutral-200">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.25em]">الإدارة العامة للمرور - محافظة صنعاء</span>
        </div>
      </div>
    </div>
  );
