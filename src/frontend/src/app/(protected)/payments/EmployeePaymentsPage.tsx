...
export default function EmployeePaymentsPage() {
  ...
  const { data: paymentsData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => paymentService.getAllPayments(page, 20, statusFilter !== '' ? statusFilter.toString() : undefined, searchQuery || undefined),
    staleTime: 30000,
  });
  ...
  // Calculate stats
  const totalAmount = payments.filter(p => p.status === PaymentStatus.Paid).reduce((sum, p) => sum + p.amount, 0);
...
{/* KPI Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-4">
          {[
            ...
            { label: 'عمليات ناجحة', value: payments.filter(p => p.status === PaymentStatus.Paid).length, icon: ShieldCheck, color: 'text-[#1a3a8f]', bg: 'bg-[#1a3a8f]/5', border: 'border-[#1a3a8f]/10' },
            { label: 'مرفوضة / مرتجعة', value: payments.filter(p => p.status === PaymentStatus.Failed || p.status === PaymentStatus.Refunded).length, icon: AlertCircle, color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' },
          ].map((stat, i) => (
        ...
        <div className="flex items-center gap-3">
           <div className="flex items-center gap-2 bg-neutral-50 px-4 py-1.5 rounded-xl border border-neutral-200">
              ...
              >
                ...
                <option value={PaymentStatus.Paid}>تم التحصيل</option>
                <option value={PaymentStatus.Pending}>قيد المراجعة</option>
                <option value={PaymentStatus.Failed}>عمليات فاشلة</option>
                <option value={PaymentStatus.Refunded}>ملغاة / مرتجعة</option>
              </select>
           </div>
              ...
              <table className="w-full text-right border-collapse">
                <thead>
                  <tr className="bg-neutral-50/50">
                    ...
                    <th className="px-8 py-6 text-[10px] font-black text-neutral-400 uppercase tracking-[0.25em] border-b border-neutral-100 text-end">الإجراء</th>
                  </tr>
                </thead>
                  ...
                  {filteredPayments.map((p, index) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-6 text-xs font-bold text-neutral-500">
                        {p.feeType === FeeType.ApplicationFee ? "فتح الملف" : "الفحص الطبي"}
                      </td>
                      <td className="px-8 py-6">
