...
export default function UpgradePage() {
  ...
  const loadLicenses = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const activeLicenses = response.data.filter(l => l.status === LicenseStatus.Active);
    ...
    const fetchLicenses = async () => {
      try {
        ...
        if (isMounted && response.success && response.data) {
          const activeLicenses = response.data.filter(l => String(l.status) === 'Active');
          setLicenses(activeLicenses);
        }
      } catch (err) {
        if (isMounted) {
  ...
  const loadUpgradeTargets = async (licenseId: number) => {
    try {
