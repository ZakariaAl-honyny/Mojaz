...
function StatCard({ title, value, change, icon: Icon, index, locale }: StatCardProps) {
  return (
...
export default function UsersReportPage() {
  ...
  const nationalityData: NationalityData[] = [
    { nationality: locale === 'ar' ? 'اليمنية' : 'Yemeni', count: 6250, percentage: 73.2 },
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={registrationTrend}>
                  ...
                  <Tooltip
                    contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
                  />
                  <Bar dataKey="registrations" fill="#1E3A8A" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
