...
test.describe('Applicant Authentication Flow', () => {
  ...
  const testEmail = `${testId}@DrivingLicenseIssuanceSystem.gov.sa`;
  ...
  test('US1: Registration and OTP integration', async ({ page }) => {
    ...
    await DrivingLicenseIssuanceSystemUtils.measurePerformance(page, 'Registration Submit', async () => {
  ...
  test('US1: Existing User Login', async ({ page }) => {
    ...
    await page.getByTestId('login-identifier').fill('applicant@DrivingLicenseIssuanceSystem.gov.sa');
    await page.getByTestId('login-password').fill('Password123!');

    await DrivingLicenseIssuanceSystemUtils.measurePerformance(page, 'Login Submit', async () => {
