...
public class RenewalServiceTests
{
    ...
    public async Task ValidateEligibilityAsync_WithExpiredLicenseOutsideGracePeriod_ReturnsNotEligible()
    {
        ...
        _licenseRepositoryMock
            .Setup(x => x.FindAsync(
                It.Is<Expression<Func<License, bool>>>(expr => 
                    expr.Compile()(license)), // Only return the license if it matches the predicate
    ...
    public async Task CreateRenewalAsync_WithValidRequest_ReturnsCreatedApplication()
    {
        ...
        _renewalApplicationRepositoryMock
            ...
            .ReturnsAsync((RenewalApplication r, CancellationToken _) => 
            {
                r.Id = 1; // Set an ID as if it was generated by the database
                return r;
            });
        ...
        // Assert - check success first
        result.Success.Should().BeTrue();
        
        // Verify the service returned non-empty Guid by checking via the mock
        _renewalApplicationRepositoryMock.Verify(
            x => x.AddAsync(It.IsAny<RenewalApplication>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    {
        // Arrange
        var holder = new User { Id = 10, FullNameAr = "Test User" };
        var category = new LicenseCategory { Id = 11, NameAr = "B", NameEn = "B", Code = LicenseCategoryCode.B, ValidityYears = 10 };
