...
- [X] **T201**: Initialize Next.js 15 project in `src/frontend` with App Router, TypeScript, ESLint, and Tailwind CSS.
- [X] **T202**: Configure tailwind v4 with Mojaz theme in globals.css (replaces tailwind.config.ts)
- [X] **T203**: Install core dependencies: `next-intl`, `next-themes`, `zustand`, `@tanstack/react-query`, `axios`, `zod`, `react-hook-form`, `lucide-react`, `clsx`, `tailwind-merge`.
- [X] **T204**: Configure shadcn/ui and add base components: `Button`, `Card`, `Input`, `DropdownMenu`, `Dialog`, `Skeleton`.
- [X] **T205**: Add global fonts: IBM Plex Sans Arabic (Google Fonts) and Inter.

## Phase 2: i18n & Layout Structure (Priority: P1)

- [X] **T206**: Configure `next-intl` middleware and `i18n.ts` config for `/ar` and `/en`.
- [X] **T207**: Update `src/app/[locale]/layout.tsx` to handle dynamic `dir` and `lang` based on route params.
- [X] **T208**: Implement `next-themes` provider and wrap root layout.
- [X] **T209**: Create `LanguageSwitcher` and `ThemeToggle` components.
- [X] **T210**: Create `PublicLayout` and `DashboardLayout` base components.

## Phase 3: Infrastructure & Services (Priority: P2)

- [X] **T211**: Setup `src/lib/api-client.ts` with Axios instance, base URL, and interceptors for Bearer token.
- [X] **T212**: Implement `src/lib/utils.ts` with `cn()`, `formatDate()`, and `formatCurrency()`.
- [X] **T213**: Setup `src/providers/QueryProvider.tsx` with TanStack React Query.
- [X] **T214**: Create `src/stores/auth-store.ts` using Zustand with persistence.
- [X] **T215**: Define global TypeScript types in `src/types/` (ApiResponse, User, LicenseCategory).

## Phase 4: Finalization & Verification (Priority: P2)

- [X] **T216**: Create a bilingual `not-found.tsx` page.
- [X] **T217**: Verify local development with `npm run dev`.
- [X] **T218**: Run `npm run build` to ensure no build-time errors.
- [X] **T219**: Audit accessibility and responsive design on the base layout.

## Success Criteria Checklist

- [X] App loads at `localhost:3000/ar` with RTL layout.
- [X] Locale switching works instantly.
- [X] Dark/Light mode persists.
- [X] API client handles tokens correctly.
...
- [X] T029 Run Playwright E2E test verifying language toggle, theme toggle, and public layout display.
- [X] T030 Ensure 0 strict TypeScript errors using `npm run build`
